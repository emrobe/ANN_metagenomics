...
HMMSEARCH = '/home/emr023/AI_taxclass_prestage/bin/hmmsearch'
...
def do_search(inp, profile, matches, do_trim=True, reverse=False, cpu=1):
	# Do HMM search
	### Add support for fastq using pipe with awk -> hmmer (if test for fasta/fastq)
	if (args.f):
		#pipe
		awk = """awk '{if(NR%4==1) {printf(">%s\n",substr($0,2));} else if(NR%4==2) print;}' """
		fastq = subprocess.popen(awk+inp, stdout = subprocess.PIPE)
		inp = '-'
...
if __name__ == '__main__':
	...
	parser.add_argument('-f', help="Fastq as input", action="store_true")
	...
	if (len(matches.keys())==0):
		sys.exit('Found 0 rrna sequences in '+args.input)
