...
class Tree:
    ...
    def pruneUnassigned(self):
        ...
        for node in toRemove:
            try:
                ...
                sys.stderr.write("Warning: Could not remove node %s\n" %
                                 node.name)
    ...
    def printPopulationsAtDepth(self, depth=PHYLUM, outFile=None,
                                ...
                                normaliseToBase=True, outputNovel=False,
                                useRankData=True):
        ...
        for node in nodes:
            if "Unknown" in node.name and not outputNovel:
                pass
            else:
...
class Node:
    ...
    def assignRead(self, read, dataset=None, primary=False, recursive=False):
        assignment = self.getAssignment(dataset)
        if assignment:
            assignment.addRead(read, primary)
        else:
            assignment = Assignment(read, primary)
            self.assignments[dataset] = assignment
        if recursive:
            if self.parent:
                self.parent.assignRead(read, dataset,
    ...
    def getPhylogenyRDPStyle(self, root=True, limit=False, newTabStyle=False):
        ...
        #TODO Change to tab style if it exists
...
class Assignment:
    ...
    def __init__(self, read, primary=False):
        self.singletons = None
        self.doubletons = None
        self.population = 0
        self.reads = []
        self.primReads = []
        self.addRead(read, primary)

    def addRead(self, read, primary=False):
        self.reads += [read]
        if primary:
            self.primReads += [read]
        if "_" in read.name:
            try:
                readPopulation = int(read.name.split("_")[-1].replace(".00",
                                                                      ""))
            except:
                readPopulation = 1
        elif "numreads=" in read.name:
            readPopulation = int(read.name[read.name.find("numreads=") +
                                           len("numreads="):])
        else:
            readPopulation = None
